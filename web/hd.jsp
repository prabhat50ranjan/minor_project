<%-- 
    Document   : hd
    Created on : May 3, 2017, 9:10:18 AM
...
<center>GRAPH WITH 1 YEAR DATA AND OPENING PRICE</center>  
    <div ng-view></div>
    ...
    angular.module('AngularChartExample',['AngularChart'], function( $routeProvider, $locationProvider ){
        $routeProvider.when('/',{
            template: '<chart title="1 MONTH OPENPRICE" xData="lineChartXData" yData="lineChartYData" xName="Month" yName="openprice" subtitle=""></chart>',
            controller: MainCtrl
            })
    })
    </script>
...
try
         ...
         Document doc = Jsoup.connect("https://in.finance.yahoo.com/quote/HD?p=HD").get();
...
out.write("</td>");
   out.write("<td>");
   try{
  FileReader fr=new FileReader("E:\\hd.csv");
